/**
 * planningCore.ts — Shared planning logic (Node + Deno compatible)
 *
 * This file is the authoritative source of all macro-planning logic.
 * It has no Supabase calls, no fetch, no LLM calls.
 * It is imported by macroPlan.ts (Node/app) and re-exported by
 * supabase/functions/_shared/planningCore.ts (Deno edge functions).
 */

import { addDays, differenceInDays, format, parseISO } from 'date-fns';
import constitutionData from './constitutionData';
import type { ConstitutionData } from './constitutionData';
import type {
  Phase,
  EventDemandProfile,
  SessionPurpose,
